...
export class Guard {
  ...
  async setGuardConfig(algorithm: string, iteractions: number) {
    ...
    assert(iteractions >= 0, INCORRECT_CONFIG_PARAMS);
    assert(iteractions % 2 === 0, INCORRECT_CONFIG_PARAMS);
  ...
  async setLogOutTimer(timer: number) {
    assert(TypeOf.isInt(timer), TIMER_MUST_BE_INT);
    assert(timer > 0, TIMER_MUST_BE_INT);
  ...
  async exportMnemonic(password: string) {
    assert(this.isReady, WALLET_NOT_READY);

    try {
      assert(Boolean(this.#encryptMnemonic), WALLET_NOT_SYNC);
  ...
  async unlock(password: string) {
    ...
    try {
      ...
      assert(mnemonicController.validateMnemonic(mnemonic), INCORRECT_PASSWORD);
  ...
  checkSession() {
    assert(this.#isReady, WALLET_NOT_READY);
    assert(this.#isEnable, WALLET_NOT_ENABLED);
  }
